...
export class BasePage {
     ...
     * @param distance - Swipe distance (0-1)
     */
    async swipeHorizontal(direction: 'left' | 'right', distance: number = 0.5): Promise<void> {
        const { width, height } = await driver.getWindowSize();
     ...
     * @param selector - Element selector
     */
    async isElementDisplayed(selector: string, timeout: number = 10000): Promise<boolean> {
        try {
            console.log(`Checking if element is displayed: ${selector} (timeout: ${timeout}ms)`);
